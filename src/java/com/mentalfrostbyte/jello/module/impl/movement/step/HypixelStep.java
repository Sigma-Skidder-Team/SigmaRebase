...
public class HypixelStep extends Module {
    ...
    public void onEnable() {
        ...
        this.field23994 = 0;
        sendBlinkingStatusMessage(); // Send initial blinking status message
    }
    ...
    private void stepUpEvent(EventStep var1) {
        if (this.isEnabled() && !var1.isCancelled()) {
            ...
            sendBlinkingStatusMessage();
        }
    }

    private void sendBlinkingStatusMessage() {
        if (isBlinking) {
            Class8906.method32487("Blinking is now enabled.");
        } else {
            Class8906.method32487("Blinking is now disabled.");
        }
    }
