...
public class MovementAntiBot extends Class7249 {
   ...
   public boolean method22756(Entity var1) {
      if (!mc.world.getBlockState(var1.getPosition()).isSolid()) {
         ...
         for (BlockPos pos : getBlockPositionsInAABB(var1)) {
            VoxelShape var7 = mc.world.getBlockState(pos).getShape(mc.world, pos);
            if (!var7.isEmpty() && var4.intersects(var7.getBoundingBox().offset(pos))) {
               return true;
            }
         }
   ...
   public static List<BlockPos> getBlockPositionsInAABB(Entity var0) {
