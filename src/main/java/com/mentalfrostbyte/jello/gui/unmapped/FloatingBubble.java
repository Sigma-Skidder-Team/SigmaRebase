...
public class FloatingBubble extends CustomGuiScreen {
    ...
    public void updatePanelDimensions(int newHeight, int newWidth) {
        ...
        if (!(this.field20932 + (float) this.widthA < 0.0F)) {
            if (this.field20932 > (float) MinecraftClient.getInstance().getMainWindow().getWidth()) {
                this.field20932 = (float) (0 - this.widthA);
            }
        } else {
            this.field20932 = (float) MinecraftClient.getInstance().getMainWindow().getWidth();
        }

        if (!(this.field20933 + (float) this.heightA < 0.0F)) {
            if (this.field20933 > (float) MinecraftClient.getInstance().getMainWindow().getHeight()) {
                this.field20933 = (float) (0 - this.heightA);
            }
        } else {
            this.field20933 = (float) MinecraftClient.getInstance().getMainWindow().getHeight();
        }
