...
public class CustomLoadingScreen extends LoadingGui {
    ...
    public CustomLoadingScreen(Minecraft client, IAsyncReloader monitor, Consumer<Optional<Throwable>> exceptionHandler,
            boolean reloading) {
        this.client = client;
        ...
        background = Resources.createScaledAndProcessedTexture2("com/mentalfrostbyte/gui/resources/loading/back.png",
                0.25F, 25);
    }
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        if (this.client.getMainWindow().getWidth() != 0) {
            var17 = (float) (this.client.getMainWindow().getFramebufferWidth()
                    / this.client.getMainWindow().getWidth());
        }

        float var18 = (float) this.client.getMainWindow().calcGuiScale(this.client.gameSettings.guiScale,
        ...
        if (this.applyCompleteTime == -1L && this.reloadMonitor.fullyDone() && (!this.reloading || var12 >= 2.0F)) {
            ...
            if (this.client.currentScreen != null) {
                this.client.currentScreen.init(this.client, this.client.getMainWindow().getScaledWidth(),
                        this.client.getMainWindow().getScaledHeight());
            }
        }
    }
    ...
    public static void xd(float var0, float var1) {
        GL11.glEnable(3008);
        GL11.glEnable(3042);
        RenderUtil.drawImage(0.0F, 0.0F, (float) MinecraftClient.getInstance().getMainWindow().getWidth(),
                (float) MinecraftClient.getInstance().getMainWindow().getHeight(), background, var0);
        RenderUtil.drawRoundedRect2(0.0F, 0.0F, (float) MinecraftClient.getInstance().getMainWindow().getWidth(),
                (float) MinecraftClient.getInstance().getMainWindow().getHeight(), ColorUtils.applyAlpha(0, 0.75F));
        int var4 = 455;
        int var5 = 78;
        int var6 = (MinecraftClient.getInstance().getMainWindow().getWidth() - var4) / 2;
        int var7 = Math
                .round((float) ((MinecraftClient.getInstance().getMainWindow().getHeight() - var5) / 2) - 14.0F * var0);
        float var8 = 0.75F + var0 * var0 * var0 * var0 * 0.25F;
        GL11.glPushMatrix();
        GL11.glTranslatef((float) (MinecraftClient.getInstance().getMainWindow().getWidth() / 2),
                (float) (MinecraftClient.getInstance().getMainWindow().getHeight() / 2), 0.0F);
