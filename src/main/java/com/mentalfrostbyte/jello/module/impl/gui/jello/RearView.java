        ...
        if (framebuffer != null) {
            if (this.isEnabled()) {
                if (! Minecraft.getInstance().gameSettings.hideGUI) {
                    if (!this.getBooleanValueFromSettingName("Smart Visibility")) {
                        this.animation.changeDirection(mc.currentScreen != null && !this.getBooleanValueFromSettingName("Show in GUI") ? Animation.Direction.BACKWARDS : Animation.Direction.FORWARDS);
                    } else {
                    ...
                    if (this.animation.calcPercent() == 0.0F && this.animation.calcPercent() == 1.0F) {
                        ...
                        positionY = (int) ((float) positionY * SmoothInterpolator.interpolate(this.animation.calcPercent(), 0.49, 0.59, 0.16, 1.04));
                    } else {
                        positionY = (int) ((float) positionY * SmoothInterpolator.interpolate(this.animation.calcPercent(), 0.3, 0.88, 0.47, 1.0));
                    }
