...
public class MineplexHighJump extends Module {
    ...
    public void onMove(EventMove event) {
        if (this.isEnabled()) {
            ...
            if (!mc.player.isOnGround() && this.field24023) {
                ...
                MovementUtil.setMotion(event, this.field24024);
                ...
                if (Math.abs(event.getY()) < var4 && Math.abs(this.field24025) < var4) {
                    this.field24025 -= 0.04000000000001;
                    event.setY(this.field24025);
                    mc.player.setMotion(mc.player.getMotion().x, event.getY(), mc.player.getMotion().z);
                } else {
                    this.field24025 = event.getY();
                }
            }
        }
    }
    ...
    public double method16975(AxisAlignedBB var1) {
        ...
        do {
            bb = var1.offset(0.0, -var4, 0.0);
            if (mc.world.getCollisionShapes(mc.player, bb).count() == 0L) {
                return var4;
            }

            var4 -= 0.5;
        } while (mc.world.getCollisionShapes(mc.player, bb).count() != 0L && var4 > 3.0);
